...
<form action="index_gv.php" method="post" class="search-tutor" enctype="multipart/form-data">
      ...
      <button type="button" class="inline-btn" onclick="them()">Thêm giáo viên</button>
   </div>
...
<div class="box-container">
        ...
        <?php
           ...
           while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo '<a href="edit_gv.php?id=' . $row['id'] . '" class="inline-btn">Sửa</a>';
            echo '&ensp;';
            echo '<a href="delete_gv.php?id=' . $row['id'] . '" class="inline-btn">Xóa</a>';
            echo '</div>';
            $columnCount++;
            if ($columnCount >= 5) {
                        $columnCount = 0; 
